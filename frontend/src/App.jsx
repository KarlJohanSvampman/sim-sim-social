...
function Character({ c, onSelect }) {
  ...
  const angle = Math.atan2(facing.x || 0, facing.y || 1);
  const name = c.name || c.profile?.name || "Unknown";
  const spokenText = c.spoken_text || c.state?.spoken_text || "";
  const thoughts = c.thoughts || c.state?.current_action_name || c.state?.mood || "...";
  const app = c.appearance_summary || {
    age: c.profile?.age,
    sex: c.profile?.sex,
    body_type: c.profile?.appearance_tags?.[0]?.tag || "unknown"
  };

  return (
    <group onClick={(e) => { e.stopPropagation(); onSelect && onSelect(); }} rotation={[0, angle, 0]} position={[c.position.x, 0.65, c.position.y]}>
      <mesh>
      ...
      <Html position={[0, 1.0, 0]} center>
        <div style={{ background: "#fff", padding: "4px 6px", borderRadius: 6, border: "1px solid #999", fontSize: 12, maxWidth: 220 }}>
          ...
          {spokenText ? (
            <div style={{ marginTop: 4, background: "#fef3c7", border: "1px solid #f59e0b", padding: "4px 6px", borderRadius: 8 }}>
              🗨️ {spokenText}
            </div>
          ) : null}
  ...
  return (
    <div style={{ height: "calc(100vh - 56px)", display: "grid", gridTemplateColumns: "1fr 320px" }}>
      <div style={{ position: "relative" }}>
        ...
        <Canvas camera={{ position: [18, 24, 18], fov: 50 }} onPointerMissed={(e) => { if (e.type === "click") setSelectedTile(null); }}>
          ...
          {Object.values(world.characters || {}).map((c) => (
            <Character key={c.id || `${c.position?.x}-${c.position?.y}`} c={c} onSelect={() => {}} />
          ))}
          {Object.values(world.tagged_characters || {}).map((c) => (
            <Character
              key={c.profile.id}
              c={{
                profile: c.profile,
                ...
                name: c.profile.name,
                position: c.position,
                thoughts: c.state.current_action_name || c.state.mood,
                spoken_text: c.state.spoken_text,
                appearance_summary: {
                  age: c.profile.age,
                  sex: c.profile.sex,
                  body_type: c.profile.appearance_tags?.[0]?.tag || "unknown"
      ...
      <div style={{ background: "#111827", color: "#fff", padding: 16, overflow: "auto" }}>
        ...
        <div>Tagged characters: {Object.keys(world.tagged_characters || {}).length}</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <ErrorBoundary resetKey={page}>{renderPage()}</ErrorBoundary>
    </div>
  );
}
