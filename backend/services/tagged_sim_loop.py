...
def _advance_calendar(world: dict) -> None:
    calendar = world.setdefault(
        "calendar",
        {"year": 2026, "month": 4, "day": 16, "minute_of_day": 480},
    )
...
def pick_social_target(c):
    ...
    for o in TAGGED_CHARACTERS.values():
        ...
        score += float(c.state.affinity.get(o.profile.id, 0.0))
        score -= _grudge_intensity_against(c, o.profile.id)

        if c.state.household_id and c.state.household_id == o.state.household_id:
            score += 8.0

        kws = _view_keywords_for_subject(c, o.profile.id)
        if "nice" in kws or "warm" in kws or "interesting" in kws:
            score += 5.0
        if "annoying" in kws or "threatening" in kws or "untrustworthy" in kws:
            score -= 6.0

        dx = o.position["x"] - c.position["x"]
        dy = o.position["y"] - c.position["y"]
        dist_penalty = abs(dx) + abs(dy)
        score -= dist_penalty * 0.25
...
def _clear_turn_state(a, b=None) -> None:
