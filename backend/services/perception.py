...
def perceive(character, world):
    ...
    for hazard in world.get("hazards", []):
        d=sqrt((me["x"]-hazard["location"]["x"])**2+(me["y"]-hazard["location"]["y"])**2)
        if d<=character["sight_radius"]:
            out.append({"type":"visual","source_id":f'hazard:{hazard["type"]}',"position":hazard["location"],"certainty":max(0.2,1.0-d/character["sight_radius"]),"data":{"entity_type":"hazard","appearance":hazard["type"],"action":"burning"}})
    return out
